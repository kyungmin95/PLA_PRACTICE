...
public class WeekEditTodoActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //일정들을 추가한 뒤 체크박스 모양(확인)을 누르면 인텐트 종료
    ...
    public void makeList() {    //선택 배경 색에 해당하는 데이터들을 가져와 리스트를 설정
        ...
        Cursor c = db.rawQuery("select _id, content from plaweektodo_ex where date = '"+ wd +
                "' AND color = '" + col + "'", null);
        while(c.moveToNext()) {
    ...
    public void editTodo(View v) { //+, 수정 버튼을 누를때 실행. DB에 내용을 삽입, 수정 하고 리스트 다시 재설정
        ...
        String str_cont;
        switch(v.getId()) {
            case R.id.weektodoe_add:
                str_cont = eEdit.getText().toString();
                if (str_cont.length() == 0)
                    break;
                String queryadd = String.format("insert into %s values(null, '%s', '%s', '%s', '%s', '%s');",
                        "plaweektodo_ex", wd, str_cont, "F", col, "O");
                db.execSQL(queryadd);
                break;
            case R.id.weektodoe_upd:
                str_cont = eEdit.getText().toString();
                if (str_cont.length() == 0)
                    break;
                String queryupd = String.format("update %s set content='%s' where _id = %d;", "plaweektodo_ex", str_cont, dei.getEd_id());
                db.execSQL(queryupd);
                break;
        }
        eEdit.setText("");
