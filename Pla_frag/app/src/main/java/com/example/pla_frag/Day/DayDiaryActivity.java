...
public class DayDiaryActivity extends Activity {
    ...
    public void editDiary(View v) {  // 체크 모양을 누르면 메모 내용을 DB에 추가하거나 수정하고 intent를 종료함
        ...
        Cursor c = db.rawQuery("select _id from pladaydi_ex where date = '"+ dd +"';", null);
        while(c.moveToNext()) {
            mId = c.getInt(0);
        }
        ...
        if(mecont.length() != 0) {
            if(mId == -1) { //DB 탐색 결과가 없으면 해당 내용을 DB에 insert함
                String queryadd = String.format("insert into %s values(null, '%s', '%s');", "pladaydi_ex", dd, mecont);
                db.execSQL(queryadd);
            }
            else {  //DB 탐색 결과가 있으면 해당 내용을 DB에 update 함
                String queryupd = String.format("update %s set dicont='%s' where _id = %d;", "pladaydi_ex", mecont, mId);
                db.execSQL(queryupd);
            }
        }
        else {      //mecont 에 내용이 아무것도 없으면 DB 내용 삭제
            db.delete("pladaydi_ex", "_id=" + mId, null);
        }
        db.close();
        c.close();
        //날짜를 전달하면서 intent 종료
