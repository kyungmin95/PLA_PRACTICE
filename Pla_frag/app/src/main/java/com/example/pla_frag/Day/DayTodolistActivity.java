...
public class DayTodolistActivity extends Activity {
    ...
    public void makeList() {    //디비에서 날짜에 해당하는 내용을 가져와 리스트에 추가하고 어댑터에 알려주는 함수
        ...
        Cursor c = db.rawQuery("select _id, content from pladaytodo_e where date = '"+ dd +"';", null);
        while(c.moveToNext()) {
        ...
        switch(v.getId()) {
            case R.id.daytodoe_add:
                ...
                if (str_cont.length() == 0)
                    break;
                String queryadd = String.format("insert into %s values(null, '%s', '%s', '%s');", "pladaytodo_e", dd, str_cont, "F");
                db.execSQL(queryadd);
                break;
            case R.id.daytodoe_upd:
                ...
                if (str_cont.length() == 0)
                    break;
                String queryupd = String.format("update %s set content='%s' where _id = %d;", "pladaytodo_e", str_cont, dei.getEd_id());
                db.execSQL(queryupd);
                break;
        }
        editCont.setText("");
