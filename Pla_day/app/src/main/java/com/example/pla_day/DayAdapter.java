        ...
        chb.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(isChecked == true) {
                    ...
                    String queryupd = String.format("update %s set checked='%s' where _id = %d;", "pladaytodo_exc", "T", mList.getDId());
                    db.execSQL(queryupd);
                    db.close();
                    txtv.setPaintFlags(txtv.getPaintFlags() | Paint.STRIKE_THRU_TEXT_FLAG);
                }
                    ...
                    DayDB helper = new DayDB(context);
                    SQLiteDatabase db = helper.getWritableDatabase();
                    String queryupd = String.format("update %s set checked='%s' where _id = %d;", "pladaytodo_exc", "F", mList.getDId());
                    db.execSQL(queryupd);
                    db.close();
                    txtv.setPaintFlags(txtv.getPaintFlags() ^ Paint.STRIKE_THRU_TEXT_FLAG);
                }
